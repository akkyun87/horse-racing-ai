...
【Usage】
    ...
    logger = setup_logger(
        log_filepath="logs/data_loader.log",
        log_level="INFO",
        logger_name="DataLoader",
    )
...
HORSE_ID_PATTERN: Final[re.Pattern[str]] = re.compile(r"/horse/(\d+)/")

# JBIS のレース結果 URL から開催日 (YYYYMMDD) を抽出するパターン
# 例: "/race/result/20250601/105/11/" → "20250601"
RACE_DATE_PATTERN: Final[re.Pattern[str]] = re.compile(r"/race/result/(\d{8})/")

# レース名からグレード・クラス区分を抽出するパターン
# 対象: G1〜G3, JG1〜JG3 (障害), Listed, オープン/OP
GRADE_PATTERN: Final[re.Pattern[str]] = re.compile(
    r"(G[I]{1,3}|J\s?G[I]{1,3}|L|オープン|OP)"
)

# 一般条件クラスのキーワード（グレード未該当時の検索順）
_CLASS_KEYWORDS: Final[List[str]] = [
    "新馬",
    "未勝利",
    "1勝クラス",
    "2勝クラス",
...
def _save_race_urls(
    ...
    Returns:
        None: 戻り値なし（副作用として JSON ファイルを書き出す）。
...
def fetch_and_store_race_and_pedigree_data(
    ...
    Example:
        >>> logger = setup_logger("logs/data_loader.log", logger_name="DataLoader")
        >>> fetch_and_store_race_and_pedigree_data(logger=logger)
    """
    ...
    if tests:
        ...
        all_urls = [
            "https://www.jbis.or.jp/race/result/20250601/105/11/",  # ダービー
            "https://www.jbis.or.jp/race/result/20251228/106/11/",  # 有馬記念
        ]
    else:
...
def _run_tests() -> None:
    ...
    外部依存（DB・ネットワーク・YAML）はダミーデータで代替し、
    テストで生成したファイルはすべて `finally` 節で削除する。
    単体実行が可能。
    ...
    TEST_LOG_DIR: Final[str] = "logs/_test_data_loader_tmp"
    TEST_LOG_FILE: Final[str] = f"{TEST_LOG_DIR}/test.log"
    TEST_LOGGER_NAME: Final[str] = "test_data_loader"
    # TEST 6 で _save_race_urls の出力先として使用する一時ディレクトリ
    TEST_URL_SAVE_DIR: Final[str] = f"{TEST_LOG_DIR}/urls"
    ...
    try:
        # ---------------------------------------------------------
        # テスト 1: _split_date_range 正常系 (3ヶ月分割)
        # ---------------------------------------------------------
        print("\n[TEST 1] 正常系: _split_date_range の3ヶ月分割")
        ranges = _split_date_range("2024-01-01", "2024-03-15")
        assert len(ranges) == 3, f"分割件数が一致しません: {len(ranges)} (期待値: 3)"
        assert ranges[0] == (
            "2024-01-01",
            "2024-01-31",
        ), f"1月範囲が一致しません: {ranges[0]}"
        assert ranges[2] == (
            "2024-03-01",
            "2024-03-15",
        ), f"3月範囲が一致しません: {ranges[2]}"
        print("  -> PASS")

        # ---------------------------------------------------------
        # テスト 2: _split_date_range 単月範囲
        # ---------------------------------------------------------
        print("\n[TEST 2] 正常系: _split_date_range の単月範囲")
        ranges_single = _split_date_range("2024-05-10", "2024-05-20")
        assert (
            len(ranges_single) == 1
        ), f"分割件数が一致しません: {len(ranges_single)} (期待値: 1)"
        print("  -> PASS")

        # ---------------------------------------------------------
        # テスト 3: _extract_year_month_from_url (正常系)
        # ---------------------------------------------------------
        print("\n[TEST 3] 正常系: _extract_year_month_from_url の年月抽出")
        y, m = _extract_year_month_from_url(
            "https://www.jbis.or.jp/race/result/20250801/13/11/"
        )
        assert y == "2025", f"year が一致しません: {y!r} (期待値: '2025')"
        assert m == "8", f"month が一致しません: {m!r} (期待値: '8')"
        print("  -> PASS")

        # ---------------------------------------------------------
        # テスト 4: _extract_year_month_from_url (異常系・パターン不一致)
        # ---------------------------------------------------------
        print("\n[TEST 4] 異常系: _extract_year_month_from_url の不正URL で ValueError")
        try:
            _extract_year_month_from_url("https://www.jbis.or.jp/horse/2021100001/")
            assert False, "ValueError が発生しませんでした"
        except ValueError:
            # 意図: 日付パターンが含まれない URL は ValueError を送出することを確認する
            pass
        print("  -> PASS")
        ...
        # ---------------------------------------------------------
        print("\n[TEST 5] 正常系: _filter_unique_entries の DB 未存在時全件返却")
        ...
        ), f"DB 未存在時に全件が返却されませんでした: {result}"
        print("  -> PASS")

        # ---------------------------------------------------------
        # テスト 6: _save_race_urls (正常系・JSON ファイル生成確認)
        # ---------------------------------------------------------
        print("\n[TEST 6] 正常系: _save_race_urls の JSON ファイル生成")
        # 意図: TEST_LOG_DIR 配下に URL 保存先を置くことで finally で一括削除できる
        test_urls = [
        ...
        _save_race_urls(
            ...
            "2024-05-01",
            logger,
            base_dir=Path(TEST_URL_SAVE_DIR),
        )
        saved_files = list(Path(TEST_URL_SAVE_DIR).rglob("*.json"))
        assert (
            len(saved_files) >= 1
        ), f"JSON ファイルが生成されませんでした (検索先: {TEST_URL_SAVE_DIR})"
        print("  -> PASS")

        # ---------------------------------------------------------
        # テスト 7: _build_race_key (フォーマット確認)
        # ---------------------------------------------------------
        print("\n[TEST 7] 正常系: _build_race_key のキーフォーマット検証")

        # 意図: RaceDetail の全フィールドを用意せず、key 生成に必要な属性のみを持つ
        #       ダミーオブジェクトで検証する
        class _MockVenue:
            place = "東京"
        ...
        assert (
            key == "2024-05-26_東京_11"
        ), f"生成キーが一致しません: {key!r} (期待値: '2024-05-26_東京_11')"
        print("  -> PASS")
